...
-- FleetGraph queue/run/delivery foundation.
-- Findings themselves are documents with document_type = 'fleetgraph_finding'.
...
-- FleetGraph indexes
CREATE INDEX IF NOT EXISTS idx_fleetgraph_event_queue_ready ON fleetgraph_event_queue(available_at, created_at)
...
CREATE INDEX IF NOT EXISTS idx_fleetgraph_action_proposals_run ON fleetgraph_action_proposals(run_id, created_at DESC)
  WHERE run_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_fleetgraph_action_proposals_target ON fleetgraph_action_proposals(target_document_id, created_at DESC)
